...
class Job extends Model
{
    ...
    public function storeJobs($request)
    {
        ...
        $json = array();
        if (!empty($request)) {
            $random_number = Helper::generateJobCode();
            ...
            $this->genre = filter_var($request['genre'], FILTER_SANITIZE_STRING);
            $this->total_no_word = filter_var($request['total_no_word'], FILTER_SANITIZE_STRING);
            ...
            $this->project_type = filter_var($request['project_type'] ?? 'fixed', FILTER_SANITIZE_STRING);
            $this->flexible_days_req = $this->project_type == 'flexible' ? filter_var($request['flexible_days_req'], FILTER_SANITIZE_STRING) : null;
    ...
    public function updateJobs($request, $id)
    {
        ...
        if (!empty($request)) {
            ...
            $job->genre = filter_var($request['genre'], FILTER_SANITIZE_STRING);
            $job->total_no_word = filter_var($request['total_no_word'], FILTER_SANITIZE_STRING);
            ...
            $job->project_type = filter_var($request['project_type'] ?? 'fixed', FILTER_SANITIZE_STRING);
            $job->flexible_days_req = $job->project_type == 'flexible' ? filter_var($request['flexible_days_req'], FILTER_SANITIZE_STRING) : null;
            if (Schema::hasColumn('jobs', 'expiry_date')) {
    ...
    public static function getSearchResult(
        ...
        if (!empty($projectTypes)) {
            ...
            $jobs->whereIn('project_type', is_array($projectTypes) ? $projectTypes : []);
        }
