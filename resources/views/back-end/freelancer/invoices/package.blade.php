                                    ...
                                    @foreach ($invoices as $invoice)
                                        @php
                                            ...
                                            if ($package_options['duration'] === 10){
                                                    $expiry_date = (!empty($invoice) && !empty($invoice->created_at)) ?\Carbon\Carbon::parse( $invoice->created_at)->addDays(4) : '';
                                            } elseif ($package_options['duration'] === 30){
                                                    $expiry_date = (!empty($invoice) && !empty($invoice->created_at)) ? \Carbon\Carbon::parse( $invoice->created_at)->addDays(30): '';
                                            } elseif ($package_options['duration'] === 360){
                                                    $expiry_date = (!empty($invoice) && !empty($invoice->created_at)) ? \Carbon\Carbon::parse( $invoice->created_at)->addDays(360) : '';
                                            }
                                        @endphp
                                        @if (!empty($invoice))
                                            <tr>
