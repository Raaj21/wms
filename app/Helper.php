...
class Helper extends Model
{
    ...
    public static function uploadTempImage($temp_path, $image, $file_name = "")
    {
        ...
        if (!empty($image)) {
            ...
            if ($extension === "jpg" || $extension === "png" || $extension === "gif" || $extension === "jpge" || $extension === "webps") {
     ...
     * @return json response
     */
    public static function uploadProfileImageWithSize($temp_path, $image, $file_name = "", $image_size = array())
    {
        $json = array();
        if (!empty($image)) {
            $file_original_name = $image->getClientOriginalName();
    ...
    public static function getProjectLevel($key = "")
    {
        $list = array(
            ...
            'expert' => trans('lang.project_level.expert'),
        );
        if (!empty($key) && array_key_exists($key, $list)) {
            return $list[$key];
        } else {
    ...
    public static function getHourlyRate($key = "")
    {
        ...
        $list = array(
            '00-05' => trans('lang.freelancer_hourly_rate.0_5', ['currency' => $symbol]),
            '05-10' => trans('lang.freelancer_hourly_rate.5_10', ['currency' => $symbol]),
    ...
    public static function getUserName($user_id)
    {
        if (!empty($user_id) && !empty(User::find($user_id))) {
            ...
            //return date('md',strtotime(User::find($user_id)->expiry_date)).User::find($user_id)->location_id;
            return date('Y',strtotime(User::find($user_id)->created_at)).date('md',strtotime(User::find($user_id)->expiry_date)).$user_id;
        } else {
            return ;
        }
    }
        ...
        if (!empty($role)) {
            if ($role == 'client') {
                $list = array(
                    ...
                    '5' => trans('lang.emp_pkg_opt.image'),
                    '6' => trans('lang.emp_pkg_opt.pvt_cht'),
                );
            } elseif ($role == 'writer') {
                $list = array(
                    ...
                    '2' => trans('lang.freelancer_pkg_opt.pkg_duration'),
                    '3' => trans('lang.freelancer_pkg_opt.badge'),
                    '4' => trans('lang.freelancer_pkg_opt.banner'),
                    '5' => trans('lang.freelancer_pkg_opt.pvt_cht'),
                );
            }
            return $list;
        }
    ...
    public static function getSearchableList($type)
    {
        $json = array();
        if ($type == 'writer') {
            $freelancs = User::role('writer')->select(
        ...
        if ($type == 'client') {
            $employers = User::role('client')->select(
                DB::raw("CONCAT(users.first_name,' ',users.last_name) AS name"),
                "slug"
            )->where('is_disabled', 'false')->get()->toArray();
            $json = $employers;
        ...
        if ($type == 'service') {
            $services = DB::table("services")
                ->select(
                    "title AS name",
                    "slug"
                )->where('status', 'published')->get()->toArray();
            $json = $services;
        }
        return $json;
    }
            ...
            if (!empty($location)) {
                $services = DB::table("services")
                    ->select(
                        "title AS name",
                        "slug"
                    )->where('location_id', $location)->where('status', 'published')->get()->toArray();
            } else {
                $services = DB::table("services")
                    ->select(
                        "title AS name",
    ...
    {
        $message = !empty($message) ? $message : trans('lang.restricted_text');
        if (isset($_SERVER["SERVER_NAME"]) && $_SERVER["SERVER_NAME"] === 'wms.com') {
    ...
    {
        if (Auth::user()) {
            if (Auth::user()->getRoleNames()->first() == 'writer') {
    ...
    public static function generateJobCode() {
        $randJobCode = rand(1000000000, 9999999999);

        $jobs = DB::table('jobs')->select('code')->where('code', $randJobCode)->get();

        return !empty($jobs) && count($jobs) > 0 ? self::generateJobCode() : $randJobCode;
    }

    public static function getProjectLevelCost()
    {
        $list = array(
            'basic'     => .01,
            'medium'    => .11,
    ...
    public static function calculateElapsedTime($timeFrom, $timeTo = null) {
        $timeTo = $timeTo ?? time();
        ...
        foreach($condition as $secs => $str) {
            ...
            if($d >= 1) {
                ...
                return $d . ' ' . $str . ($d > 1 ? 's' : '') . ' ' . ($diff > 0 ? ' ago' : ' remaining');
            }
        }
    }
}
