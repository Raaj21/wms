...
class JobController extends Controller
{
    ...
    public function postJob()
    {
        ...
        $projectLevelsCost = Helper::getProjectLevelCost();
    ...
    public function store(Request $request)
    {
        ...
        if (Auth::user()->user_verified == 1) {
            ...
            if (Schema::hasColumn('jobs', 'expiry_date')) {
                ...
                if ($request['project_type'] == 'flexible' && empty($request->get('flexible_days_req'))) {
                    $json['type'] = 'error';
                    $json['message'] = trans('lang.flexible_days_req');
                    return $json;
                }
                ...
                if ($expiry->lessThan(Carbon::now())) {
                    ...
                    return $json;
                } else if ($expiry->greaterThan(Carbon::now()->addDays(10))) {
                    $json['type'] = 'error';
                    $json['message'] = trans('lang.future_expiry_date');
                    return $json;
                }
    ...
    public function show($slug)
    {
        ...
        if (!empty($job)) {
            ...
            $user_role_key = !empty(Auth::user()->profile->tagline) ? str_limit(Auth::user()->profile->tagline, 26, '') : Helper::getAuthRoleName();
            $user_role = (trim($user_role_key) == 'Writer') ? 'Writer' : 'Client';
