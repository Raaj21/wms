														...
														<b>{{$invoice->payment_mode}}</b>
															...
															<div class="wt-freelancers-content">
																<div class="dc-title">
																	...
																	@endif
																	<h3>{{{$title}}} <span> <a class="print-window wt-addinfo " target='_blank' href="{{ url('show/invoice/'.$invoice->id) }}">{{trans('lang.order')}}#{{$order->id}}</a></span><span>{{$order->payment_mode}}</span> </h3>
																	<span><strong>{{ !empty($symbol) ? $symbol['symbol'] : '$' }}{{{$amount}}}</strong></span>
																</div>
															</div>
															<span>
																{{{ Carbon\Carbon::parse($invoice->created_at)->diffForHumans()}}} on {{{Carbon\Carbon::parse($invoice->created_at)->format('l \\a\\t H:i:s')}}}
															</span>
														</div>
													</span>
															...
															<div class="wt-service-tabel">
																...
																<div class="wt-freelancers-content">
																	<div class="dc-title">
																		...
																		@endif
																		<a href="{{{url('profile/'.encrypt($user->slug))}}}"><h3>{{{Helper::getUserName($user->id)}}}</h3></a>
																	</div>
																	<div class ="dc-title">
																	<span >{{(Helper::getRoleByUserID($user->id) == '2') ? 'Client' : 'Writer' }}</span>
																	 </div>
																</div>
															</div>
														</span>
