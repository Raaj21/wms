                        ...
                        {{-- @if (!empty($types))
                            <div class="wt-checkboxholder wt-verticalscrollbar">
                                @foreach ($types as $key => $ty)
                                    @php $checked = ( !empty($_GET['types']) && in_array($ty->slug, $_GET['types'])) ? 'checked' : '' @endphp
                                    <span class="wt-checkbox">
                                        <input id="types-{{{ $key }}}" type="checkbox" name="types[]" value="{{{$ty->slug}}}" {{$checked}} >
                                        <label for="types-{{{ $key }}}">{{{ $ty->title }}}</label>
                                    </span>
                                @endforeach
                            </div>
                        @endif --}}
                        <div class="wt-radioboxholder wt-verticalscrollbar">
                            @foreach ($jobTypes as $jobTypeId => $jobTypeName)
                                @php $checked = ( !empty($_GET['jobType']) && $jobTypeId ==  $_GET['jobType']) ? 'checked' : '' @endphp
                                <span class="wt-radio">
                                    <input id="jobType-{{{ $jobTypeId }}}" type="radio" name="jobType" value="{{{ $jobTypeId }}}" {{$checked}} >
                                    <label for="jobType-{{{ $jobTypeId }}}">{{{ $jobTypeName }}}</label>
                                </span>
                            @endforeach
                        </div>
                    </fieldset>
                    ...
                    @if (!empty($genres))
                        <div class="wt-checkboxholder wt-verticalscrollbar">
                            @foreach ($genres as $key => $genre)
                                @php $checked = ( !empty($_GET['genre']) && in_array($genre->slug, $_GET['genre'])) ? 'checked' : '' @endphp
                                <span class="wt-checkbox">
                                    <input id="genre-{{{ $key }}}" type="checkbox" name="genre[]" value="{{{$genre->slug}}}" {{$checked}} >
                                    <label for="genre-{{{ $key }}}">{{{ $genre->title }}}</label>
                                </span>
                            @endforeach
                        </div>
                    @endif
