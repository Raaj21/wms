...
class UserController extends Controller
{
    ...
    public function generateOrder($id, $type)
    {
        ...
        if (!empty($id)) {
            ...
            if ($type == 'service') {
                if (!empty($new_order['type']) && $new_order['type'] == 'error') {
                    $json['type'] = 'error';
                    $json['process'] =  $new_order['message'];
                    return $json;
                }
    ...
    public function getEmployerInvoices($type = '')
    {
        ...
        }else if (Auth::user()->getRoleNames()[0] != 'admin' && Auth::user()->getRoleNames()[0] === 'client') {
            //dd(Auth::user()->id);
            $currency   = SiteManagement::getMetaValue('commision');
            $symbol = !empty($currency) && !empty($currency[0]['currency']) ? Helper::currencyList($currency[0]['currency']) : array();
            $invoices = array();
        ...
        if (!empty($id)) {
            ...
            if (Auth::user()->getRoleNames()->first() === 'client') {
                ...
                if (file_exists(resource_path('views/extend/back-end/employer/invoices/show.blade.php'))) {
                    ...
                    return view::make('back-end.admin.orders.show', compact('invoice_info', 'symbol', 'currency_code'));
                }
            }
        }
    }
    ...
    public function updatePayoutStatus(Request $request)
    {
        ...
        if (!empty($request['id'])) {
            ...
            $payout->save();
            if (!empty($request['projects_ids']) && $request['status'] == 'completed') {
