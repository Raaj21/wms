                        ...
                        <div class="col-xs-12 col-sm-12 col-md-7 col-lg-7 col-xl-8 float-left">
                            <div class="wt-userlistingholder wt-userlisting wt-haslayout">
                                <div class="wt-userlistingtitle">
                                    @if (!empty($users))
                                        <span>{{ trans('lang.01') }} {{$users->count()}} of {{\App\User::role('writer')->count()}} results @if (!empty($keyword)) for <em>"{{{$keyword}}}"</em> @endif</span>
                                    @endif
                                </div>
                                @if (!empty($users))
                                    ...
                                    @foreach ($users as $key => $freelancer)
                                       ...
                                       $slug = Helper::getUserslug($freelancer->id);
                                         $enurl = (isset($freelancer->id)) ? encrypt($slug) : '#';
                                        ...
                                        <div class="wt-userlistinghold {{ $feature_class }}">
                                                ...
                                                <div class="wt-contenthead">
                                                    <div class="wt-title">
                                                        ...
                                                        @if (!empty($freelancer->profile->tagline))
                                                            <h2><a href="{{{ url('profile/'.$enurl) }}}">{{{ $freelancer->profile->tagline }}}</a></h2>
                                                        @endif
                                                    </div>
                                            ...
                                            @endif
                                            @if (!empty($freelancer->profile->type))
                                             @php
                                                         $search_type =$freelancer->profile->type; 
                                                        ...
                                                        @if(!empty($types))
                                                          ...
                                                          <span><i class="far fa-folder"></i>
                                                                {{ (!empty($typesdata->title)) ? $typesdata->title : '' }}</span>
                                                           
                                                             @endforeach
                                                             </div>
                                                        @endif
                                                        @endif
                                                         @if (!empty($freelancer->profile->languages))
                                                         @php
                                                         $search_languages =$freelancer->profile->languages;
                                                         if (!empty($search_languages)) {
                                                         $filters = strpos($search_languages, '{{') === 0 ? json_decode($search_languages) : explode(',', $search_languages);
                                                         $languages  = array();
                                                         if(isset($filters)){
                                                         $languages = \App\Language::whereIn('id',$filters)->get();
                                                        }
                                                         ...
                                                         @endphp
                                                         @if(!empty($languages))
                                                            <div><i class="far fa fa-language"></i> Languages: 
                                                           @foreach( $languages as $li => $languagedata)
                                                         
                                                            <span class="">{{ (!empty($languagedata)) ? $languagedata->title : ''  }}</span>@if($li < (count($languages) - 1)),@endif
                                                          
                                                            @endforeach
                                                            </div>
                                                        @endif
                                                        @endif
                                            @if (!empty($freelancer->profile->genre))
                                                @php
                                                    $search_genres = $freelancer->profile->genre; 

                                                    if (!empty($search_genres)) {
                                                        $genresFilter = strpos($search_genres, '{{') === 0 || strpos($search_genres, '[') === 0 ? json_decode($search_genres) : explode(',', $search_genres);
                                                        $genres  = array();

                                                        if(isset($genresFilter)){
                                                            $genres = \App\Genre::whereIn('id', $genresFilter)->get();
                                                        }
                                                    }
                                                @endphp

                                                @if(!empty($genres))
                                                    <div class="mt-2"><i class="far fa-list-alt"></i> Genres: 
                                                        @foreach( $genres as $gi => $genreData)
                                                        <span class="">{{ (!empty($genreData)) ? $genreData->title : ''  }}</span>@if($gi < (count($genres) - 1)),@endif
                                                        @endforeach
                                                    </div>
                                                @endif
                                            @endif
                                            @if(isset($invite) && !empty($invite))
                                                <div class="mt-2 float-right">
                                                    <a href="{{ str_replace('WRITER_SLUG', $freelancer->slug, $invite) }}" class="wt-btn">Invite to Job</a>
                                                </div>
                                            @endif
                                        </div>
                                    @endforeach
