...
class ProposalController extends Controller
{
            ...
            if (!empty($request)) {
                ...
                if (!empty($payment_settings) && $package_status === 'true') {
                    ...
                    if ($proposals >= $allowed_proposals) {
                        ...
                        if ($submit_propsal['type'] = 'success') {
                            $json['type'] = 'success';
                            $json['message'] = trans('lang.proposal_submitted');
                            $user = User::find(Auth::user()->id);

                            if($request['invited'] == 'yes') {
                                if($job->freelancer_invited == auth()->user()->id) {
                                    $request->merge([
                                        'id' => $submit_propsal['submittedProposalId'],
                                    ]);

                                    $json = $this->hiredFreelencer($request);
                                    return $json;
                                }
                            }
                            ...
                            if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                                if (!empty($job->employer->email)) {
                                    ...
                                    if (!empty($proposal_received_template->id)
                                        ...
                                        $email_params['duration'] = $request['completion_time'] . ' days';
    ...
    public function sendJobInvitationToWriter($jobSlug, $writerSlug) {
        if(!empty($jobSlug) && !empty($writerSlug)) {
            $job = \App\Job::where('slug', $jobSlug)->first();

            if(!empty($job) && auth()->check() && $job->user_id == auth()->user()->id) {
                $writer = \App\User::where('slug', $writerSlug)->first();

                if(!empty($writer) && $writer->getRoleNames()->first() == 'writer') {
                    $job->freelancer_invited = auth()->user()->id;
                    $job->save();

                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        if(!empty($writer->email)) {
                            $emailParams = [
                                'freelancer_name' => Helper::getUserName($writer->id),
                                'employer_name' => Helper::getUserName(auth()->user()->id),
                                'project_link' => url('job/' . $jobSlug),
                                'project_title' => $job->duration,
                                'project_amount' => $job->price,
                                'project_duration' => $job->duration,
                                'accept_invitation_link' => url('writer/job/' . $jobSlug . '/accept-invitation'),
                            ];
                            $emailContent = '<p>Hello <strong>%freelancer_name%</strong>,</p>
<p><strong> %employer_name%</strong> has sent you an invitation on the following project <a href="%project_link%"><strong>%project_title%</strong></a>. Project Information is given below.</p>
<p><strong>Project Amount :</strong> %project_amount%</p>
<p><strong>Project Duration :</strong> %project_duration%</p>
<p>To accept the Invitation, click below link:</p>
<p><strong><a href="%accept_invitation_link%">Accept Invitation</a></strong></p>';

                            foreach($emailParams as $var => $value) {
                                $emailContent = str_replace('%' . $var . '%', $value, $emailContent);
                            }

                            $subject = 'Job Invitation';
                            $to = $writer->email;
                            $fromEmail = \App\EmailHelper::getEmailFrom();
                            $fromEmailId = \App\EmailHelper::getEmailID();

                            Mail::send([], [], function($message) use ($to, $fromEmail, $fromEmailId, $subject, $emailContent) {
                                $message->from($fromEmail, $fromEmailId);
                                $message->to($to);
