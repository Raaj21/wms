...
class Profile extends Model
{
    ...
    protected $fillable = [
        ...
        'profile_searchable', 'profile_blocked', 'weekly_alerts', 'message_alerts', 'education1', 'experience1','languages','type','genre'
    ];
    ...
    public function storeProfile($request, $user_id)
    {
        ...
        $ed_lt = '';
        if(isset($request['education1']) && !empty($request['education1'])){
            foreach($request['education1'] as $ed){
                $ed_lt .=$ed.',';
            }
        }
        $ln_lt = '';
        if(isset($request['languages']) && !empty($request['languages'])){
            foreach($request['languages'] as $ln){
                $ln_lt .=$ln.',';
            }
        }
        ...
        $profile->genre = $genre_lt;
        $profile->type = $type_lt;
        $profile->languages = $ln_lt;
        $profile->experience1 = $request['experience1'];
        $profile->education1= $ed_lt;
