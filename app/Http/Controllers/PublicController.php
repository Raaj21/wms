...
class PublicController extends Controller
{
    ...
    public function showUserProfile($slug)
    {
        ...
        if (!empty($user)) {
            ...
            if ($user->getRoleNames()->first() === 'writer') {
                $services = array();
                ...
                if(!empty($user->profile->type)) {
                    $userTypeIds = $user->profile->type;
                    $types = \App\Type::whereIn('id', ( strpos($userTypeIds, '{{') === 0 || strpos($userTypeIds, '[') === 0 ? json_encode($userTypeIds) : explode(',', $userTypeIds) ))->get();
                }
            ...
            } elseif ($user->getRoleNames()->first() === 'client') {
    ...
    public function getSearchResult($search_type = "")
    {
        ...
        $locations  = Location::all();
        $selectlocations = Location::pluck('title', 'id');
        $selectlanguages = Language::pluck('title', 'id');
        $selecttype = Type::pluck('title', 'id');
        $selectgenres  = Genre::pluck('title', 'id');
        ...
        $search_type = !empty($_GET['types']) ? $_GET['types'] : array();
        
        $search_genres = !empty($_GET['genre']) ? $_GET['genre'] : array();
        ...
        if (!empty($_GET['type'])) {
            if ($type == 'client' || $type == 'writer') {
                ...
                if ($type === 'client') {
                    ...
                    if(!empty($invite)) {
                        $invJob = \App\Job::where('slug', $invite)->first();

                        if(!empty($invJob) && auth()->check() && $invJob->user_id == auth()->user()->id) {
                            $invite = url('client/job/' . $invJob->slug . '/invite-writer/WRITER_SLUG');
                        } else {
                            $invite = null;
                        }
                    }
                ...
                $results = Job::getSearchResult(
                    ...
                    $min_price,
                    $max_price,
                    (isset($_GET['jobType']) && !empty($_GET['jobType']) ? [$_GET['jobType']] : []),
                );
                
                $jobs = $results['jobs'];
                $jobTypes = Helper::getProjectTypeList();
                $jobTypes = $jobTypes ?? [];

                if(isset($jobTypes['hourly'])) {
                    unset($jobTypes['hourly']);
                }
                // dd($jobs[0]);
                if (!empty($jobs)) {
    ...
    {
        $json = array();
        if (Auth::user() && Auth::user()->getRoleNames()->first() === 'client') {
            $json['auth'] = true;
            return $json;
        } else {
            $json['auth'] = false;
