...
@section('content')
<style>
.project_cost::-webkit-input-placeholder { padding-left:15px; }
.project_cost::placeholder { padding-left:15px; }
.project_cost:focus{padding-left:35px;}
.inputWithIcon [name="project_cost"] { padding-left: 30px;  }
</style>
                                    ...
                                    <fieldset>
                                        ...
                                        <div class="form-group form-group-half wt-formwithlabel">
                                            <span class="wt-select">
                                                {!! Form::select('genre', $genre, null, array('class' => '', 'placeholder' => 'Genre', )) !!}
                                            </span>
                                        </div>
                                        <div class="form-group form-group-half wt-formwithlabel job-cost-input inputWithIcon">
                                            {!! Form::text('total_no_word', null, array('class' => 'total_no_word', 'placeholder' => 'Total Number of words', 'onkeypress'=>'return IsNumeric(event);')) !!}
                                        ...
                                        <div class="form-group form-group-half wt-formwithlabel job-cost-input inputWithIcon">
                                            ...
                                            {!! Form::text('project_cost', null, array('class' => 'project_cost', 'placeholder' => trans('lang.project_cost'), 'onkeypress'=>'return IsNumeric(event);')) !!}
                                        ...
                                        <div class="form-group form-group-half">
                                            <div class="">
                                                ...
                                                <span class="wt-radio">
                                                    <input type="radio" name="project_type" id="project_type-flexible" value="flexible">
                                                    <label for="project_type-flexible">Flexible</label>
                                                </span>
                                                <span class="wt-radio mt-1">
                                                    <input type="radio" name="project_type" id="project_type-fixed" value="fixed">
                                                    <label for="project_type-fixed">Fixed</label>
                                                </span>
                                            </div>
                                        </div>
                                        <div class="form-group form-group-half wt-formwithlabel d-none">
                                            {!! Form::text('flexible_days_req', null, array('class' => 'form-control flexible_days_req', 'placeholder' => 'No of days required to complete the Job', 'onkeypress'=>'return IsNumeric(event);')) !!}
                                        </div>
                                    </fieldset>
                                </div>
                            </div>
...
@section('bootstrap_script')
    ...
    <script>
        const projectLevelsCost = {!! json_encode($projectLevelsCost ?? []) !!};
...
function IsNumeric(e) {
    ...
    function calculateProjectCost() {
        let totalWords = $('[name="total_no_word"]').val();
        let level = $('[name="project_levels"]').val();
        let costPerWord = projectLevelsCost[level] || 0;
        console.log(costPerWord);

        if(costPerWord > 0) {
            $('[name="project_cost"]').val(costPerWord * totalWords);
        }
    }

    $(document).on('change', '[name="total_no_word"], [name="project_levels"]', function() {
        calculateProjectCost();
    });

    $(document).on('input', '[name="total_no_word"]', function() {
        calculateProjectCost();
    });
