...
use App\Review;
use App\Language;
use App\Genre, App\Type, App\Education;
...
class FreelancerController extends Controller
{
    ...
    public function index()
    {
        ...
        $education = Education::pluck('title', 'id');
        ...
        $user_experience = !empty($profile->experience1) ? $profile->experience1 : '';
        $user_education = !empty($profile->education1) ? (explode(",",$profile->education1)): '';
        $user_languages = !empty($profile->languages) ? (explode(",",$profile->languages)) : '';
        $user_genre = !empty($profile->genre) ? (explode(",",$profile->genre)) : '';
        $user_type = !empty($profile->type) ? (explode(",",$profile->type)) : '';
    ...
    public function storeProjectAwardSettings(Request $request)
    {
        ...
        if (!empty($request)) {
            ...
            $awards = $request->get('award');
            $awardInvalidMessage = null;

            if(!empty($awards)) {
                foreach($awards as $award) {
                    $awardInvalidMessage = (pathinfo($award['award_hidden_image'], PATHINFO_EXTENSION) == 'pdf' ? null : $award['award_title'] . ' File must only be a PDF file.');

                    if(!empty($awardInvalidMessage)) {
                        break;
                    }
                }
            }
            ...
            if(!empty($awardInvalidMessage)) {
                $json['type'] = 'error';
                $json['message'] = $awardInvalidMessage;
    ...
    public function acceptJobInvitation($jobSlug) {
        if(!empty($jobSlug)) {
            if(auth()->check() && auth()->user()->getRoleNames()->first() == 'writer') {
                $job = \App\Job::where('slug', $jobSlug)->first();

                if(!empty($job) && $job->freelancer_invited == auth()->user()->id && $job->status == 'posted') {
                    return redirect()->to( route('createProposal', ['job_slug' => $job->slug]) . '?invited=yes' );
                }
