...
class User extends Authenticatable
{
    ...
    public static function getSearchResult(
        ...
        $users = !empty($user_by_role) ? User::whereIn('users.id', $user_by_role)->where('users.is_disabled', 'false')->join('profiles','users.id','profiles.user_id') : array();
        ...
        if (!empty($users)) {
            ...
            if (!empty($keyword)) {
                $filters['s'] = $keyword;
                $users->where('users.first_name', 'like', '%' . $keyword . '%');
                $users->orWhere('users.last_name', 'like', '%' . $keyword . '%');
                $users->orWhere('users.slug', 'like', '%' . $keyword . '%');
                $users->whereIn('users.id', $user_by_role);
                $users->where('users.is_disabled', 'false');
            }
            if (!empty($search_locations)) {
                ...
                $users->whereIn('users.location_id', $locations);
            }
            ...
            if (!empty($search_genres)) {
                ...
                $filters['genres'] = $search_genres;
                $genre = Genre::whereIn('slug', $search_genres)->get();
                ...
                $users->whereIn('profiles.genre', $g_id);
                // $users->whereRaw('FIND_IN_SET("'.$g_id.'",profiles.genre)');//whereRaw(FIND_IN_SET($g_id, 'profiles.genre'));
                //whereIn('profiles.genre', $g_id);
            }
            if (!empty($search_type)) {
               
                $filters['type'] = $search_type;
                $type = Type::whereIn('slug', $search_type)->get();
                foreach ($type as $key => $t) {
                    if (!empty($t->id)) {
                        $t_id[] = $t->id;
                    }
                }
                $users->whereIn('profiles.type', $t_id);
            }
            if ($type = 'writer') {
                $users = $users->orderByRaw('users.badge_id','-', 'DESC')->orderBy('users.expiry_date', 'DESC');
            } else {
